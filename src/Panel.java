...
public class Panel extends JPanel implements ActionListener {
    ...
    Button[] selectPlayer = new Button[17];
    static int player_1_chose = -1, player_2_chose = -1;

    Pointer selectTurn = new Pointer(1);
    Keyboard keyboard = new Keyboard(sw, selectTurn);
    ...
    Panel() {
        ...
        for (int i = 1; i <= 8; i++)
            lostObject[i - 1] = new ImageIcon("img/lostObject/lostObject_" + i + ".png").getImage();

        for (int i = 1; i <= 4; i++)
            weapon[i - 1] = new ImageIcon("img/weapon/inventory/weapon" + i + ".png").getImage();

        for (int i = 0; i < 15; i++)
            pImage[i] = new ImageIcon("img/hero/player" + i + ".png").getImage();

        for (int i = 0; i < 15; i++)
            pSmallImage[i] = new ImageIcon("img/hero/small/player" + i + ".png").getImage();

        for (int i = 1; i <= 6; i++)
            diceImage[i - 1] = new ImageIcon("img/dice/dice" + i + ".png").getImage();

        for (int i = 0; i < 5; i++)
            endIcon[i] = new ImageIcon("img/end/icon" + i + ".png").getImage();

        for (int i = 0; i < 5; i++) {
            for (int j = 0; j < 3; j++) {
                selectPlayer[3 * i + j] = new Button(60 + i * 235, 55 + j * 220, 200, 200, "", null, pImage[3 * i + j]);
                int finalI = i, finalJ = j;
                selectPlayer[3 * i + j].addMouseListener(new MouseListener() {
                    @Override
                    public void mouseClicked(MouseEvent e) {
                        int i = finalI;
                        int j = finalJ;
                        if (selectTurn.prt == 1 && player_2_chose != 3 * i + j) {
                            player1Image = pImage[3 * i + j];
                            player1 = pSmallImage[3 * i + j];
                            player_1_chose = 3 * i + j;
                        }
                        if (selectTurn.prt == 2 && player_1_chose != 3 * i + j) {
                            player2Image = pImage[3 * i + j];
                            player2 = pSmallImage[3 * i + j];
                            player_2_chose = 3 * i + j;
                        }
                    }
    ...
    public void paint(Graphics g) {
        ...
        switch (sw.prt) {
            ...
            case 2:
                ...
                for (int i = 0; i < 5; i++) {
                    for (int j = 0; j < 3; j++) {
                        this.remove(selectPlayer[3 * i + j]);
                    }
                }
            ...
            case 3://end
                ...
                if (player1Info.treasureNumber == player2Info.treasureNumber) {
                    ...
                    if (player2Info.treasure[0].show)
                        g2D.drawImage(treasure[0], 200 - 130 + 330, 345 - 50, null);
                    if (player2Info.treasure[1].show)
                        g2D.drawImage(treasure[1], 278 - 130 + 330, 340 - 50, null);
                    if (player2Info.treasure[2].show)
                        g2D.drawImage(treasure[2], 338 - 130 + 330, 340 - 50, null);
                    if (player2Info.treasure[3].show)
                        g2D.drawImage(treasure[3], 400 - 130 + 330, 340 - 50, null);
                    if (player2Info.treasure[4].show)
                        g2D.drawImage(treasure[4], 475 - 390 + 330, 340 + 40, null);
                    if (player2Info.treasure[5].show)
                        g2D.drawImage(treasure[5], 525 - 390 + 330, 340 + 40, null);
                    if (player2Info.treasure[6].show)
                        g2D.drawImage(treasure[6], 602 - 390 + 330, 345 + 40, null);
                    if (player2Info.treasure[7].show)
                        g2D.drawImage(treasure[7], 655 - 390 + 330, 340 + 40, null);

                    for (int i = 0; i < 4; i++)
                        for (int j = 0; j < 2; j++)
                            g2D.drawRoundRect(70 + i * 65, 290 + j * 90, 50, 70, 10, 10);
                    for (int i = 0; i < 4; i++)
                        for (int j = 0; j < 2; j++)
                            g2D.drawRoundRect(400 + i * 65, 290 + j * 90, 50, 70, 10, 10);
